...
class TieredRepresentationGet(BaseModel):
    ...
    session_id: str | None = Field(
        None, description="Optional session ID for working tier context"
    )
    target: str | None = Field(
        None,
        description="Optional peer ID to get the representation for, from the perspective of this peer",
    ...
    search_top_k: int | None = Field(
        ...
        ge=1,
        le=100,
        description="Number of semantic-search-retrieved observations to include",
    )
    search_max_distance: float | None = Field(
        None,
        ge=0.0,
...
class ObservationTier(BaseModel):
    """A single tier of observations."""
    
    explicit: list[dict] = Field(default_factory=list, description="Explicit observations")
    deductive: list[dict] = Field(default_factory=list, description="Deductive observations")
    inductive: list[dict] = Field(default_factory=list, description="Inductive observations")
    contradiction: list[dict] = Field(default_factory=list, description="Contradiction observations")


class TieredSummary(BaseModel):
    """Summary statistics for tiered representation."""
    
    working_count: int = Field(description="Number of observations in working tier")
    short_term_count: int = Field(description="Number of observations in short-term tier")
    long_term_count: int = Field(description="Number of observations in long-term tier")
    total_count: int = Field(description="Total observations across all tiers")


class TieredRepresentationResponse(BaseModel):
    """Response containing tiered user representation."""
    
    working: ObservationTier = Field(description="Current session context - real-time observations")
    short_term: ObservationTier = Field(description="Recent observations within configured time window")
    long_term: ObservationTier = Field(description="Stable user characteristics and older observations")
    summary: TieredSummary = Field(description="Summary statistics for the tiers")
    formatted: str = Field(description="Human-readable formatted representation")
