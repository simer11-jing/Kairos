...
@router.post(
    "/{peer_id}/tiered-representation",
    response_model=schemas.TieredRepresentationResponse,
    dependencies=[
        Depends(require_auth(workspace_name="workspace_id", peer_name="peer_id"))
    ],
)
async def get_tiered_representation(
    workspace_id: str = Path(...),
    peer_id: str = Path(...),
    options: schemas.TieredRepresentationGet = Body(
        ..., description="Options for getting the tiered peer representation"
    ),
):
    """Get a tiered user representation organized by temporal context.
    
    Returns observations organized into three tiers:
    - Working: Current session context (real-time)
    - Short-term: Recent observations (default 7 days)
    - Long-term: Stable user characteristics and older observations
    
    This enables agents to:
    1. Focus on immediate context (working tier)
    2. Understand recent user state (short-term tier)
    3. Access core user identity (long-term tier)
    """
    try:
        ...
        representation = await crud.get_working_representation(
            ...
            semantic_search_top_k=options.search_top_k,
            semantic_search_max_distance=options.search_max_distance,
            max_observations=settings.TIERED.MAX_LONG_TERM_OBSERVATIONS
            if settings.TIERED.MAX_LONG_TERM_OBSERVATIONS > 0
            else None,
        )
        
        # Convert to tiered representation
        from src.utils.tiered_representation import (
            create_tiered_representation,
            tiered_to_string,
            TierConfig,
        )
        
        config = TierConfig(
            short_term_days=settings.TIERED.SHORT_TERM_DAYS,
            long_term_days=settings.TIERED.LONG_TERM_DAYS,
        )
        
        tiered = create_tiered_representation(
            representation,
        ...
        # Format output
        return schemas.TieredRepresentationResponse(
            working=schemas.ObservationTier(
                explicit=[obs.model_dump() for obs in tiered.working.explicit],
                deductive=[obs.model_dump() for obs in tiered.working.deductive],
                inductive=[obs.model_dump() for obs in tiered.working.inductive],
                contradiction=[obs.model_dump() for obs in tiered.working.contradiction],
            ),
            short_term=schemas.ObservationTier(
                explicit=[obs.model_dump() for obs in tiered.short_term.explicit],
                deductive=[obs.model_dump() for obs in tiered.short_term.deductive],
                inductive=[obs.model_dump() for obs in tiered.short_term.inductive],
                contradiction=[obs.model_dump() for obs in tiered.short_term.contradiction],
            ),
            long_term=schemas.ObservationTier(
                explicit=[obs.model_dump() for obs in tiered.long_term.explicit],
                deductive=[obs.model_dump() for obs in tiered.long_term.deductive],
                inductive=[obs.model_dump() for obs in tiered.long_term.inductive],
                contradiction=[obs.model_dump() for obs in tiered.long_term.contradiction],
            ),
            summary=schemas.TieredSummary(
                working_count=tiered.working.len(),
                short_term_count=tiered.short_term.len(),
                long_term_count=tiered.long_term.len(),
                total_count=tiered.total_observations(),
            ),
            formatted=tiered_to_string(tiered),
        )
    except ValueError as e:
        logger.warning(f"Failed to get tiered representation for peer {peer_id}: {str(e)}")
        raise ResourceNotFoundException("Peer or session not found") from e
    return await search(body.query, filters=filters, limit=body.limit)
