...
def record_infer_feedback(session_id: str, adopted: bool, note: str = ""):
    ...
    if not adopted and note:
        try:
            ...
            write_to_hindsight_memory(
                f"推理反馈(不采纳): session={session_id}, 调整={note}, 原文query={last_query}",
                confidence=0.8
            )
        except: pass
...
def get_feedback_stats():
    ...
    try:
        ...
        if not feedback:
            return "暂无反馈数据"
        adopted = sum(1 for f in feedback if f.get("adopted"))
        total = len(feedback)
        return f"总反馈{total}条,采纳{adopted}条,采纳率{adopted*100//total}%"
    except:
        return "读取反馈失败"
...
请结合用户画像,给出个性化推理回答。"""
        ...
        if args.learn_from_betting:
            print("\n🎰 从投注记录学习...")
            betting_content = learn_from_betting()
            if betting_content:
                oc_memory += "\n\n=== 投注记录 ===\n" + betting_content
                print(f"   追加: {betting_content}")
                fm.update_from_text(betting_content, source="betting_results")
            else:
                print("   无投注记录")
